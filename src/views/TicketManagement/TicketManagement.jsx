...
const TicketManagement = () => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const updatedList = formList.map((row) =>
                    ...
                    <TableBody>
                      {formList?.map((entry, index) => (
                        <TableRow key={entry._id || index}>
                          ...
                          <TableCell>
                            {entry.installDate ? new Date(entry.installDate).toLocaleDateString('en-US') : ''}
                          </TableCell>
                          <TableCell>
                            {entry.expiryDate ? new Date(entry.expiryDate).toLocaleDateString('en-US') : ''}
                          </TableCell>
                          <TableCell>{entry.description}</TableCell>
                          <TableCell>{entry.urgency}</TableCell>
                          <TableCell>
                            {
                              entry?.AssignTo?.basicDetails?.firstName+' '+
                              entry?.AssignTo?.basicDetails?.lastName
                            }
                          </TableCell>
                          <TableCell>
                            <Select
                              ...
                              sx={{
                                width: 150,
                                backgroundColor:
                                  statusOptions.find((opt) => opt.TicketStatus === entry.status)?.colorCode ||
                                  '#f0f0f0',
                                color: '#fff',
                                borderRadius: 1
                              }}
