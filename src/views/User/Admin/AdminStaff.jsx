...
const AdminStaff = () => {
  ...
  const handleSearch = event => {
    ...
    const filtered = administrativeData.filter(item =>
      item?.basicDetails?.firstName?.toLowerCase().includes(query) ||
      item?.basicDetails?.lastName?.toLowerCase().includes(query) ||
      item?.basicDetails?.email?.toLowerCase().includes(query)
    )
          ...
          <Card>
            <CardHeader
              title={
                <Grid container alignItems='center' justifyContent='space-between'>
                  <Grid item>
                    {(staffPermission.Add===true || isAdmin) && <Button variant='contained' color='primary' onClick={goToAddPage}>
                      Add
                    </Button>}
                  </Grid>
                  <Grid item>
                            ...
                            return (
                              <TableRow sx={{verticalAlign: 'top'}} key={item._id}>
                                ...
                                <TableCell>
                                  ...
                                  {(staffPermission.Edit===true || isAdmin) && <EditBtn onClick={() => navigate(`/users/administrativeUpdatePage/${item._id}`)} />}
