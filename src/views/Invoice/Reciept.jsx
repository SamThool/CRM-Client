...
const AdvanceReciept = ({ closeModal, invoiceData }) => {
  ...
  const handleDelete = (index) => {
    const updated = receiptList.filter((_, i) => i !== index);
                ...
                {receiptList?.length > 0 ? (
                  receiptList.map((rec, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        <Button size="small" onClick={() => setSelectedReceiptIndex(index)} sx={{ textTransform: 'none', mr: 1 }}>
                          View
                        </Button>
                        <Button size="small" color="error" onClick={() => handleDelete(index)} sx={{ textTransform: 'none' }}>
                          Delete
                        </Button>
                      </TableCell>
                    </TableRow>
